...
export default function TaxCalculator() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      // Helper to avoid Rupee symbol issues in PDF (Standard fonts don't support it)
      const formatForPDF = (val: string) => val.replace(/₹/g, 'Rs. ');

      // Title
      page.drawText('Tax Computation Summary', {
        x: 50,
        y,
        size: 18,
        font: boldFont,
      ...
      const drawLine = (label: string, value: string, isBold = false) => {
        page.drawText(label, { x: 50, y, size: fontSize, font: isBold ? boldFont : font });
        page.drawText(formatForPDF(value), { x: 300, y, size: fontSize, font: isBold ? boldFont : font });
        y -= lineHeight;
      };

      drawLine('Assessment Year:', assessmentYear);
      drawLine('Tax Regime:', regime === 'new' ? 'New Regime' : 'Old Regime');
      y -= 10;

      drawLine('Gross Income:', formatCurrency(result.grossIncome));
      drawLine('Total Deductions:', `-${formatCurrency(result.totalDeductions)}`);
      drawLine('Taxable Income:', formatCurrency(result.taxableIncome), true);
      y -= 10;

      page.drawText('Tax Calculation:', { x: 50, y, size: fontSize, font: boldFont });
      y -= lineHeight;

      result.slabwiseTax.forEach((slab) => {
        page.drawText(formatForPDF(`${slab.slab} @ ${slab.rate}`), { x: 50, y, size: 10, font });
        page.drawText(formatForPDF(formatCurrency(slab.tax)), { x: 300, y, size: 10, font });
        y -= 15;
      });
      y -= 10;

      drawLine('Total Tax:', formatCurrency(result.totalTax));
      if (result.rebate > 0) drawLine('Rebate u/s 87A:', `-${formatCurrency(result.rebate)}`);
      if (result.surcharge > 0) drawLine('Surcharge:', formatCurrency(result.surcharge));
      drawLine('Health & Education Cess (4%):', formatCurrency(result.cess));
      y -= 10;
      
      drawLine('Net Tax Payable:', formatCurrency(result.netTax), true);

      // Footer
      y -= 40;
      page.drawText('Generated by CA Sumit Ahuja & Associates', {
        x: 50,
        y,
        size: 10,
        font,
      ...
      link.href = url;
      link.download = `Tax_Computation_${assessmentYear}.pdf`;
